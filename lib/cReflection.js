...
class StructPointer extends Type {
    ...
    /**
     * @return {number}
     * @deprecated FIXME use memory._ptrSize dynamicaly when needed
     */
    static getPtrSize() {
        switch (process.arch) {
            case 'x32':
...
Type.float = new class float extends Type {
    ...
    /** @override */
    read(memory, address) {
        return memory._ptrSize === 4 ? memory.readReal32(address) : memory.readReal64(address);
    }

    /** @override */
    write(memory, address, data) {
        return memory._ptrSize === 4 ? memory.writeReal32(address, data) : memory.writeReal64(address, data);
    }
};
